...
import json  # 导入json库
import os # 导入os库
from date.src.const import *  # 导入常量
from date.src.peashooter import *  # 导入射手类
from date.src.settings import *  # 导入设置
from date.src.background import *  # 导入背景类
from date.src.Game import *  # 导入游戏处理类
from date.src.sunlight import *  # 导入阳光类
from date.src.sunflower import *  # 导入阳光花类
from date.src.CardFrame import *  # 导入卡片框类
from date.src.plant import *  # 导入植物类
from date.src.gridPlant import *  # 导入种植提示类
from date.src.startBackground import *  # 导入开始背景类
from date.src.startButton import *  # 导入开始按钮类
from date.src.Nut import * # 导入坚果类
from date.src.PotatoMine import * # 导入土豆地雷类
from date.src.Card import * # 导入卡片类
from date.src.ChooseCardFrame import * # 导入选择卡片框类
from date.src.DisplayedCard import * # 导入显示卡片类
from date.src.reallyButton import * # 导入确认按钮类
from date.src.Shadow import * # 导入阴影类  
...
class Pvz:
    ...
    def load(self): # 加载游戏数据
        with open('date/save/map.json', 'r') as map:
            map = [list(item) for item in json.loads(map)]
        with open('date/save/gold.json', 'r') as gold:
            gold = json.load(gold.read())

    def save(self): # 保存游戏数据
        with open('date/save/map.json', 'w') as map:
            map.write(str(self.game.map) + '\n')
        with open('date/save/gold.json', 'w') as gold:
